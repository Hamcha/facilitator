...
{
	ClearExpired1(time);
	dataQueue.Push(TimeAndValue2(time, value1), __FILE__, __LINE__);
	total1 += value1;
	lastSec1 += value1;
}
...
uint64_t BPSTracker::GetBPS1(RakNetTimeUS time)
{
	ClearExpired1(time);
	return lastSec1;
}
...
struct DatagramHeaderFormat {
	...
	void Serialize(RakNet::BitStream *b)
	{
		...
		if (isACK) {
			...
			RakNetTimeMS timeMSLow = (RakNetTimeMS) sourceSystemTime & 0xFFFFFFFF;
			b->Write(timeMSLow);
			if (hasBAndAS) {
				//		b->Write(B);
				b->Write(AS);
